...
import { createGeneration } from "@/lib/mock-data"
import { recordGenerationAction } from "@/app/actions/user-actions"
...
export function GenerateClient({ initialCredits }: GenerateClientProps) {
  ...
  const simulateGeneration = async () => {
    if (!uploadedFile || !selectedStyle || currentCredits < 1) return
    ...
    try {
      ...
      // Update user credits using server action (with transaction)
      const actionResult = await recordGenerationAction(1)

      if (!actionResult.success) {
        setError(actionResult.error || "Failed to process generation")
